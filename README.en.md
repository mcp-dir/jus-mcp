...
### The Brazilian legal MCP for Claude, Cursor and AI agents

The whole Brazilian legal stack in a single connection: lawsuits by name/CPF/CNPJ/case number, official court publications, case law, judicial calculations, public tenders and audit rulings, clearance certificates, sanctions and compliance, plus 150+ queries against official sources. Platform-hosted, no login; public sources are free and only paid queries draw on prepaid credit.

- ⚖️ **One MCP, not thirty** — lawsuits, publications, case law, calculations, tenders, certificates and compliance on the SAME connection
- 🔎 **Discovery by name, CPF, CNPJ or case number**, with a consolidated X-ray (`legal_dossie`)
- 📢 **Official court publications (DJEN)** by bar number, party or case
- 📜 **Case law and binding precedents** from the higher courts
- 🧮 **16 judicial calculations** — monetary restatement, labor, FGTS, alimony, sentencing, social-security benefit
- 🏛️ **Public tenders (PNCP) and federal audit-court rulings (TCU)**
- 🧾 **Tax and labor clearance certificates** (Federal CND/PGFN, CNDT, FGTS)
- 🛡️ **Compliance and sanctions** — PEP, OFAC/UN/EU/UK, debarment lists, administrative misconduct
- 📥 **Downloads public court decisions** (appellate rulings, full text) already converted to text
- 🗂️ **150+ lookups across official bodies** — state courts, notary registries, protests, prosecution services
- ⚡ **No login, no credentials** — works on the first question
- 💳 **Public sources are free**; only paid lookups draw credit, at the SAME price as the standalone MCP
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Jus MCP` and **URL** `https://api.mcp.ai/jus`.

### Cursor

[➕ Install Jus MCP in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=jus&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvanVzIn0=)

### VS Code (Copilot Chat)

[➕ Install Jus MCP in VS Code](vscode:mcp/install?name=jus&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fjus%22%7D)
...
```
Build an X-ray of the lawsuits involving John Doe
Any court notification for bar number 21076/SP in the last 7 days?
Restate R$ 12,500 by IPCA from 01/2020 to today and compute interest
Does this CNPJ have a clear labor-debt certificate and any debarment record?
```
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/jus` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
